...
export default function USMapComponent(props: {data: GlobalData, width: number, height: number}) {
    ...
    return (
        <Box sx={{width: width, height: height}}>
            <ComposableMap projection="geoAlbers" projectionConfig={{scale: 1000}}>
                <ZoomableGroup center={[-77, 41]}>
                    <Geographies geography="/5m-US-counties.json">
                        {({ geographies }) =>
                        geographies.map((geo) => (
                            <Geography
                                key={geo.rsmKey}
